...
with_punct_txt=open('input-1.txt', 'r', encoding='utf-8').read()
...
def create_graph(uniq_word_list):
	...
	degree=np.zeros((len(uniq_word_list)))
	...
	return sorted_indices.tolist(), word_graph
	
stop_words=stopwords.words('english')
uniq_word_list=get_uniq_words(word_list, stop_words)
tagged_uniq_words = nltk.pos_tag(uniq_word_list)

word_graph, www=create_graph(uniq_word_list)
ranked_uniq_words=[uniq_word_list[i] for i in word_graph]

score=np.ones((len(uniq_word_list)))
d=0.85
